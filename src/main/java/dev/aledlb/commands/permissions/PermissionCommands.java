...
public class PermissionCommands implements CommandExecutor {
    ...
    final String header = "§3====[§bCore Permissions§3]====";

    final Core main;

    public PermissionCommands(Core main) {
        this.main = main;
    }

    private boolean addGroup(OfflinePlayer p, String arg, CommandSender sender) {
    ...
    private boolean addPermission(OfflinePlayer p, String arg, CommandSender sender) {
        sender.sendMessage(header);
        List<String> perms;
        if (p == null) {
    ...
    private boolean group(CommandSender sender, String[] args) {
        ...
        if (args[1].equalsIgnoreCase("addmember") || args[1].equalsIgnoreCase("removemember")) {
            @SuppressWarnings("deprecation") OfflinePlayer p = Bukkit.getOfflinePlayer(args[2]);
            if (p == null) {
    ...
    private boolean listPermissions(OfflinePlayer p, CommandSender sender) {
        sender.sendMessage(header);
    ...
    private boolean removeGroup(OfflinePlayer p, String arg, CommandSender sender) {
        sender.sendMessage(header);
        List<String> groups;
        groups = main.getConfig().getStringList("users." + p.getName() + ".groups");
        groups.remove(arg);
    ...
    private boolean removePermission(OfflinePlayer p, String arg, CommandSender sender) {
        sender.sendMessage(header);
        List<String> perms;
        if (p == null) {
            perms = main.getConfig().getStringList("default.permissions");
